...
void merge_lines(int first) {
  ...
  current->buffer = (char *)realloc(current->buffer, current->size + next->size);
  ...
  for (int i = first+1; i < V.lines-2; i++) {
      V.line_buff[i] = V.line_buff[i+1];
  }

  V.lines--;
  //TODO reallocate lines buffer?
}
...
void handle_backspace() {
  ...
  // delete symbol on cursor
  line *current_line = V.line_buff[V.screen_y + V.cursor_y];
  for (int i = V.screen_x + V.cursor_x-1; i < current_line->size - 2; i++) {
    current_line->buffer[i] = current_line->buffer[i+1];
  }
  current_line->size--;
